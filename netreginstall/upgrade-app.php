...
//    $upgrade_schema_files = get_upgrade_schema_files($argv[2], $argv[3]);
	...
	if(is_array ( $wp_old_tables ) && in_array( $row[0], $wp_old_tables )){
		...
		mysql_query("UPDATE ".$row[0]." SET option_name='".get_db_prefix($db_id)."user_roles' WHERE option_name='wp_user_roles';");
		break;
	    case get_db_prefix($db_id).'usermeta':
		mysql_query("UPDATE ".$row[0]." SET meta_key='".get_db_prefix($db_id)."capabilities' WHERE meta_key='wp_capabilities';");
		mysql_query("UPDATE ".$row[0]." SET meta_key='".get_db_prefix($db_id)."user_level' WHERE meta_key='wp_user_level';");
		break;
	    }
	    mysql_query("ALTER TABLE ".$row[0]." RENAME ".get_db_prefix($db_id).''.substr( $row['0'], strlen( get_db_prefix ( $db_id )), strlen($row['0']) ).";");
	}
    }
